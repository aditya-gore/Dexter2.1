...
module.exports = async function (deployer, _network, accounts) {
  ...
  let tokenArr = [dai, bat, rep, zrx];
  for (let t of tokenArr) {
    console.log(`seeding ${await t.name()} for trader1...`);
    await seedTokenBalance(t, trader1);
  }
  for (let t of tokenArr) {
    console.log(`seeding ${await t.name()} for trader2...`);
    await seedTokenBalance(t, trader2);
  }
  for (let t of tokenArr) {
    console.log(`seeding ${await t.name()} for trader3...`);
    await seedTokenBalance(t, trader3);
  }
  for (let t of tokenArr) {
    console.log(`seeding ${await t.name()} for trader4...`);
    await seedTokenBalance(t, trader4);
  }
  // await Promise.all(
  //   [dai, bat, rep, zrx].map(
